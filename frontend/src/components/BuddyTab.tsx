...
export function BuddyTab() {
  ...
  const players = useMemo(() => {
    ...
    const filtered = search.trim()
      ...
      : all;
    return [...filtered].sort((a, b) => (b.buddyScore ?? -1) - (a.buddyScore ?? -1));
  }, [data, search]);
  ...
  if (isError) {
    return <Alert severity="error">Failed to load buddy ranking</Alert>;
  }
          ...
          <Table>
            <TableHead>
              <TableRow>
                ...
                <TableCell align="right">Buddy %</TableCell>
            ...
            <TableBody>
              {players.length === 0 ? (
                ...
                players.map((player, index) => (
                  <PlayerRow key={player.playerKey} player={player} rank={index + 1} showBuddyScore />
                ))
              )}
            </TableBody>
          </Table>
